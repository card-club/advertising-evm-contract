    ...
    AuthorizedOriginReceiverUpgradeable
{
    ...
    function setNodePublicKey(
        ...
        if (
            !(msg.sender == owner() ||
                (_isTransmitter(msg.sender) && msg.sender == node))
        ) {
    ...
    function estimateCost(
        ...
        FunctionsBillingRegistryInterface.RequestBilling
            memory billing = FunctionsBillingRegistryInterface.RequestBilling(
                subscriptionId,
                msg.sender,
                gasLimit,
                gasPrice
    ...
    function _report(
        ...
        for (uint256 i = 0; i < requestIds.length; i++) {
            ...
            returns (FunctionsBillingRegistryInterface.FulfillResult result) {
                if (
                    result ==
                    FunctionsBillingRegistryInterface.FulfillResult.USER_SUCCESS
                ) {
                    emit OracleResponse(requestIds[i]);
                } else if (
                    result ==
                    FunctionsBillingRegistryInterface.FulfillResult.USER_ERROR
                ) {
