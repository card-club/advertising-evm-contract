...
const simulateRequest = async (unvalidatedConfig) => {
  ...
  const savedEnv = {
    ...
    ENABLE_CONSOLE_LOG_FROM_SANDBOX:
      process.env["ENABLE_CONSOLE_LOG_FROM_SANDBOX"],
  };
  process.env["DISABLE_TMP_CLEAR_FOR_TESTING"] = "true";
  process.env["ENABLE_CONSOLE_LOG_FROM_SANDBOX"] = "true";
  const timeout = setTimeout(() => {
    throw Error(
      "Runtime of 15 seconds for sandboxed source code has been exceeded"
    );
  }, 15000);
  const resultString = (
