...
async function promptTxCost(gasEstimate, hre, skipPrompt = false) {
  ...
  const nativePriceUSD = await getPriceUSD(
    networks.avalancheFuji.linkPriceFeed,
    hre.ethers
  );
  ...
  console.log(
    `The transaction to initiate this request will charge the wallet (${signer.address}):`
  );
  console.log(
    `${chalk.blue(
      transactionEstimateNative + " " + network.config.nativeCurrencySymbol
    )}, which ${
