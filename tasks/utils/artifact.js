...
class RequestStore {
  ...
  constructor(
    ...
    if (requestType !== "consumer" && requestType !== "automatedConsumer")
      throw new Error(
        "Unsupported request type, must be one of: consumer, automatedConsumer"
      );
  ...
  async create(data /*: RequestData*/) /*: Promise<void> */ {
    ...
    if (await this.exists(data[this.idKey])) {
      throw new Error(
        `Request ${data[this.idKey]} already exists on chain ${this.chainId}`
      );
    }
  ...
  async update(
    id /*: string*/,
    data /*: Fragment<RequestArtifactUpdateable>*/
  ) /*: Promise<void> */ {
...
function toRequestArtifact(data /*: RequestData*/) /*: RequestArtifact*/ {
  ...
  const secretsNoValues = Object.fromEntries(
    Object.entries(secrets).map(([key, value]) => [key, "[REDACTED]"])
  );
  return {
    createdAt: Date.now(),
    lastUpdatedAt: Date.now(),
...
function validateDataVersion0(data /*: RequestData*/) /*: void*/ {
  ...
  if (data.type !== "consumer" && data.type !== "automatedConsumer")
    throw new Error(
      "Request data type must be one of: consumer, automatedConsumer"
    );
  if (data.type == "consumer" && !data.requestId)
    throw new Error("Must include field: requestId");
  if (
    data.type == "automatedConsumer" &&
    !data.automatedConsumerContractAddress
  )
...
function validateRequestArtifactVersion(data /*: RequestArtifact*/) {
  if (typeof data.artifactVersion !== "string")
    throw new Error("Required field 'artifactVersion' is missing");
  if (Number(data.artifactVersion) < Number(currentVersion))
    return migrateRequestArtifactVersion(data);
  if (data.artifactVersion === currentVersion) return data;
  throw new Error(
    `Unknown version number ${data.artifactVersion}. The latest version is ${currentVersion}`
  );
}
