...
abstract contract AuthorizedOriginReceiverUpgradeable is Initializable {
     ...
     */
    function __AuthorizedOriginReceiver_initialize(
        bool active
     ...
     */
    function activateAuthorizedReceiver()
        external
     ...
     */
    function deactivateAuthorizedReceiver()
        external
        validateAuthorizedSenderSetter
    {
        if (!authorizedReceiverActive()) {
     ...
     */
    function removeAuthorizedSenders(
        address[] calldata senders
    ) external validateAuthorizedSenderSetter {
        if (senders.length == 0) {
            revert EmptySendersList();
